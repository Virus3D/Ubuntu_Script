...
class FunctionCommentSniff extends SquizFunctionCommentSniff
{
    ...
    protected function processParams(File $phpcsFile, $stackPtr, $commentStart)
    {
        ...
        // Создаем ассоциативный массив реальных параметров
        $realParamsByName = [];
        foreach ($realParams as $realParam) {
            $name = $realParam['name'];
            if ($realParam['variable_length'] === true) {
                $name = '...'.$name;
            }
            $realParamsByName[$name] = $realParam;
        }
        ...
        foreach ($params as $param) {
            ...
            // Извлекаем имя параметра из комментария
            $paramName = $param['var'];
            if ($paramName === '') {
                continue;
            }

            // Нормализуем имя (убираем & и ...)
            $normalizedName = ltrim($paramName, '&');
            $isVariadic = (substr($normalizedName, 0, 3) === '...');
            if ($isVariadic) {
                $normalizedName = substr($normalizedName, 3);
            }

            // Поиск соответствующего реального параметра
            if (!isset($realParamsByName[$paramName])) {
                // Проверяем нормализованное имя
                $paramName = $isVariadic ? '...'.$normalizedName : $normalizedName;
                if (!isset($realParamsByName[$paramName])) {
                    $error = 'Parameter %s does not exist';
                    $phpcsFile->addError($error, $param['tag'], 'InvalidParamName', [$paramName]);
                    continue;
                }
            }

            $realParam = $realParamsByName[$paramName];
            unset($realParamsByName[$paramName]);
            ...
            foreach ($typeNames as $typeName) {
                ...
                if ($suggestedTypeHint !== '' && $param['var'] !== '') {
                    $typeHint = $realParam['type_hint'];
                    ...
                    } else if ($typeHint !== 'mixed' && $typeHint !== $compareTypeHint && $typeHint !== '?'.$compareTypeHint) {
            ...
            if ($param['var'][0] === '&') {
                ...
                if ($realParam['pass_by_reference'] !== true && $realName === $paramVarName) {
